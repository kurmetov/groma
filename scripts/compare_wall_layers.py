...
things `groma layers` decodes out of `CompoundStructure`, so the two can be
joined and disagreements counted.

    groma layers model.rvt --types 4000 --links > layers.txt
...
def read_layers_report(path):
    """`groma layers --links` output: the type links and the layer tables."""
